(ns hyperspace.world
  (:require [hyperspace.misc :as misc]))
...
(defn add-random-planet
  ...
  (let [radius (-> (min width height) (/ 5) rand)
        x      (misc/rand-range (+ x radius) (- (+ x width) radius))
        y      (misc/rand-range (+ y radius) (- (+ y height) radius))]
    (assoc world
...
(defn add-random-player
  [{[x, y]          :position
    ...
    players         :players
    :as world}]
  (let [x (misc/rand-range (+ x player-radius) (- (+ x width) player-radius))
        y (misc/rand-range (+ y player-radius) (- (+ y height) player-radius))]
    (assoc world
